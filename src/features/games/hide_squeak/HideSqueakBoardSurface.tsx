...
import { getCellAtCoordinate, getItemAtCoordinate, isSameCoordinate } from './boardUtils'
...
function getHideSqueakPawTrailStepCount(
  ...
  const manhattanDistance = Math.abs(to.row - from.row) + Math.abs(to.column - from.column)
...
function getHideSqueakPawTrailStyle(step: HideSqueakPawTrailStep): CSSProperties {
  return {
    left: step.left,
    top: step.top,
      ...
      {steps.map((step) => {
        ...
        return (
          ...
          >
            <PathRevealShaftSvg
              focusable="false"
              className="absolute left-1/2 top-[30px] h-[calc(100%-30px)] w-[10px] -translate-x-1/2 text-hs-textPrimary"
            />
            <PathRevealSpearSvg
              focusable="false"
              className="absolute left-1/2 top-[5px] h-[30px] w-[30px] -translate-x-1/2 text-hs-textPrimary"
            />
          </div>
        )
      })}
...
export function HideSqueakBoardSurface({
  ...
  const columns = Array.from({ length: round.board.size.columns }, (_, index) => index + 1)
  const rows = Array.from({ length: round.board.size.rows }, (_, index) => index + 1)
  ...
  const pawTrailStepCount =
    shouldShowAnimatedCorrectAnswer && wrongAnswerCoordinate && correctAnswerCoordinate
      ? getHideSqueakPawTrailStepCount(wrongAnswerCoordinate, correctAnswerCoordinate)
      : 0
  const pawTrailSteps =
  ...
  useEffect(() => {
    ...
    const button = buttonRefs.current[formatDisplayCoordinate(activeCoordinate)]
  ...
  return (
    <div className="space-y-1">
          ...
          >
            {rows.flatMap((row) =>
              columns.map((column) => {
                ...
                const isStart = isSameCoordinate(coordinate, round.startingCoordinate)
                ...
                const shouldSuppressStartHighlight =
                  !isReviewMode &&
                  isStart &&
                  (round.difficulty === 'hard' || round.difficulty === 'super-hard')
                ...
                const showMouseToken =
                  isVisibleMouse || (showMouse && isStart && !visibleMouseCoordinate)
                ...
                const shouldShowCorrectHighlight =
                  validationResult === 'correct'
                    ? isSelected
                    : validationResult === 'wrong' || validationResult === 'revealed'
                      ? isCorrectAnswer
                      : false
                ...
                return (
                  <button
                    key={`${row}-${column}`}
                    ref={(element) => {
                      buttonRefs.current[formatDisplayCoordinate(coordinate)] = element
                    }}
                    type="button"
                    ...
                    style={{
                      borderTop: row > 1 ? '2px solid color-mix(in srgb, var(--hs-grid) 68%, transparent)' : undefined,
                      borderLeft: column > 1 ? '2px solid color-mix(in srgb, var(--hs-grid) 68%, transparent)' : undefined,
                    }}
                  >
                    {showStartLabel ? (
                      isStart ? (
                        <span
                          className={[
                            'font-game pointer-events-none absolute z-[2] font-semibold leading-none text-hs-textPrimary',
                            mousePose === 'standing'
                              ? 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-[1rem]'
                    ...
                    {item ? (
                      <HideSqueakItemAsset
                        ...
                        aria-hidden="true"
                        className={[
                          'pointer-events-none relative z-[2] h-[43%] w-[43%]',
                          isReviewMode ? 'opacity-65' : '',
                          showMouseToken ? 'translate-x-[13%] translate-y-[13%]' : '',
                        ].join(' ')}
                      />
                    ) : null}
                    ...
                    {isCorrectAnswer && !shouldShowAnimatedCorrectAnswer ? (
                      <img
                        ...
                        alt=""
                        aria-hidden="true"
                        className="pointer-events-none absolute z-[4] h-14 w-14 object-contain"
                      />
                    ) : null}

                    {cell?.kind === 'empty' && isInteractive ? (
                      <span className="sr-only">{formatDisplayCoordinate(coordinate)}</span>
                    ) : null}
                  </button>
                )
              }),
      ...
      >
        ...
        >
          {columns.map((column) => (
            <div key={`column-${column}`} className="flex items-center justify-center">
